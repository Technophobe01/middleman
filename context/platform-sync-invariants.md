...
- Archive admission requires the declared minimum cost, then bounds all wire attempts to the currently available quadratic surplus above the provider live floor. Declared and live-detail costs reserve one authentication retry per logical request plus non-GitHub feature-metadata confirmation; unknown reset timing or an uncovered floor disables archive requests. (`internal/github/budget.go::detailWorstCaseAttemptCost`,
